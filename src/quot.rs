...
pub fn check_quot<'x, 't: 'x, 'p: 't>(ctx: &'x mut TcCtx<'t, 'p>, declar: &Declar<'p>) {
    ...
    if declar.info().name == ctx.str1("Quot") {
        ...
        tc.assert_def_eq(SPtr::unshifted(declar.info().ty), SPtr::unshifted(expected_quot.info().ty));
    } else if declar.info().name == ctx.str2("Quot", "mk") {
        let env = ctx.export_file.new_env(EnvLimit::ByName(quot_mk_name));
        let mut tc = TypeChecker::new(ctx, &env, Some(*declar.info()));
        tc.assert_def_eq(SPtr::unshifted(declar.info().ty), SPtr::unshifted(expected_quot_mk.info().ty));
        ...
        let env = ctx.export_file.new_env(EnvLimit::ByName(declar.info().name));
        let mut tc = TypeChecker::new(ctx, &env, Some(*declar.info()));
        tc.assert_def_eq(SPtr::unshifted(declar.info().ty), SPtr::unshifted(expected_quot_lift.info().ty));
        return
        ...
        let env = ctx.export_file.new_env(EnvLimit::ByName(declar.info().name));
        let mut tc = TypeChecker::new(ctx, &env, Some(*declar.info()));
        tc.assert_def_eq(SPtr::unshifted(declar.info().ty), SPtr::unshifted(expected_quot_ind.info().ty));
        return
    } else {
